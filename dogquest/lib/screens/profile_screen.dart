...
class ProfileScreen extends ConsumerWidget {
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const Spacer(),
              IconButton(
                icon: const Icon(Icons.dynamic_feed_rounded, color: Colors.white54, size: 22),
                onPressed: () => context.push('/feed'),
                tooltip: 'Dog Feed',
              ),
              IconButton(
                icon: const Icon(Icons.explore_rounded, color: Colors.white54, size: 22),
                onPressed: () => context.push('/dogs-nearby'),
                tooltip: 'Dogs Nearby',
              ),
              IconButton(
                icon: const Icon(Icons.leaderboard_rounded, color: Colors.white54, size: 22),
                onPressed: () => context.push('/leaderboard'),
                tooltip: 'Leaderboard',
              ),
              IconButton(
                icon: const Icon(Icons.people_rounded, color: Colors.white54, size: 22),
          ...
          if (Hive.box('dogquest_player_stats').get('offline_mode', defaultValue: false) == true)
          ...
          Row(
            children: [
              _StatTile(icon: Icons.catching_pokemon, value: '${kennelSvc.count}', label: 'Breeds', color: Colors.amber),
              const SizedBox(width: 10),
              _StatTile(icon: Icons.visibility, value: '${sightingSvc.totalSightings}', label: 'Sightings', color: const Color(0xFFD4874E)),
              const SizedBox(width: 10),
              _StatTile(icon: Icons.emoji_events, value: '${playerState.unlockedAchievements.length}', label: 'Badges', color: const Color(0xFF7C4DFF)),
            ],
          ).animate().fadeIn(delay: 100.ms),
          const SizedBox(height: 10),
          Row(
            children: [
              _StatTile(icon: Icons.quiz, value: '${playerState.quizzesCompleted}', label: 'Quizzes', color: const Color(0xFF2196F3)),
              const SizedBox(width: 10),
              _StatTile(icon: Icons.account_tree, value: '${familySvc.completedFamilies}', label: 'Families', color: const Color(0xFFFF9800)),
              const SizedBox(width: 10),
              _StatTile(icon: Icons.workspace_premium, value: '${masteryState.totalMastered}', label: 'Mastered', color: Colors.amber),
            ],
            ...
            Row(
              children: [
                if (playerState.bestStreak > 1)
                  _MiniInfoChip(icon: Icons.military_tech, text: 'Best: ${playerState.bestStreak} days', color: Colors.orange),
                if (playerState.bestStreak > 1 && playerState.streakSavers > 0)
                  const SizedBox(width: 10),
                if (playerState.streakSavers > 0)
                  _MiniInfoChip(icon: Icons.shield, text: '${playerState.streakSavers} streak saver${playerState.streakSavers > 1 ? 's' : ''}', color: const Color(0xFFD4874E)),
              ],
          ...
          Row(
            ...
            children: [
                ...
                child: RarityCollectionWheel(
                  segments: [
                    for (final r in [Rarity.common, Rarity.uncommon, Rarity.rare, Rarity.legendary])
                      RaritySegment(
                        rarity: r,
                        collected: kennelSvc.collectedDogs.where((b) => b.rarity == r).length,
              ...
              // Mastery breakdown
              Expanded(
                child: _MasterySummary(mastery: masteryState, totalDogs: dogSvc.all.length),
              ),
            ],
          ...
          Container(
            ...
            child: CollectionHeatmap(activityData: ref.watch(activityMapProvider)),
          ...
          _SectionHeader(title: 'Achievements', icon: Icons.emoji_events, subtitle: '${playerState.unlockedAchievements.length}/${achievements.length}'),
          const SizedBox(height: 12),
          Wrap(
            ...
            children: achievements.entries.map((e) {
              ...
              return Tooltip(
                ...
                child: AnimatedContainer(
                  ...
                  width: 56, height: 56,
                  decoration: BoxDecoration(
                    color: unlocked ? Colors.amber.withValues(alpha: 0.12) : bgCard,
                    borderRadius: BorderRadius.circular(14),
                    border: Border.all(
                      color: unlocked ? Colors.amber.withValues(alpha: 0.5) : Colors.white10,
                      width: unlocked ? 1.5 : 1,
                    ),
                    boxShadow: unlocked
                        ? [BoxShadow(color: Colors.amber.withValues(alpha: 0.08), blurRadius: 8)]
                        : null,
                  ),
                  child: Center(
                    child: Text(
                      unlocked ? e.value.$1 : '\u{1F512}',
                      style: TextStyle(fontSize: 24, color: unlocked ? null : Colors.white24),
                    ),
                  ),
                ),
              );
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(16),
              border: Border.all(color: const Color(0xFFD4874E).withValues(alpha: 0.25)),
            ),
            child: Row(children: [
              Container(
                width: 44, height: 44,
                ...
                child: const Icon(Icons.eco, color: Color(0xFFD4874E), size: 22),
              ),
              const SizedBox(width: 14),
              const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Eco Impact', style: TextStyle(color: Color(0xFFD4874E), fontWeight: FontWeight.bold, fontSize: 14)),
                SizedBox(height: 2),
                Text('Every breed identified helps grow the DogQuest community.',
      ...
      widgets.add(
        Container(
          ...
          child: Row(children: [
            ...
            Text('+${((playerState.streakXpMultiplier - 1) * 100).round()}% XP from streak',
      ...
      widgets.add(
        Container(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 8),
            Expanded(
              child: Text('${event.name} — ${event.xpMultiplier}x XP (${event.daysRemaining}d left)',
                  style: TextStyle(color: event.themeColor, fontSize: 12, fontWeight: FontWeight.w600)),
            ),
          ]),
  ...
  List<Widget> _buildNextAchievements(PlayerState playerState, KennelService kennelSvc, DogService dogSvc) {
    ...
    for (final (target, key) in milestones) {
      if (!unlocked.contains(key) && count > 0) {
        ...
        if (remaining > 0 && remaining <= target) {
          hints.add(_AchievementHint(key: key, progress: count / target, hint: '$remaining more species'));
        }
        break;
      }
    }
    ...
    if (!unlocked.contains('five_rare') && rareCount > 0) {
      hints.add(_AchievementHint(key: 'five_rare', progress: rareCount / 5, hint: '${5 - rareCount} more rare'));
    }
    ...
    if (!unlocked.contains('five_legendary') && legendaryCount > 0) {
      hints.add(_AchievementHint(key: 'five_legendary', progress: legendaryCount / 5, hint: '${5 - legendaryCount} more legendary'));
    }
    ...
    for (final (target, key) in streakMilestones) {
      if (!unlocked.contains(key) && streak > 0) {
        hints.add(_AchievementHint(key: key, progress: streak / target, hint: '${target - streak} more days'));
        break;
      }
    }
    ...
    if (!unlocked.contains('ten_quizzes') && quizzes > 0 && quizzes < 10) {
      hints.add(_AchievementHint(key: 'ten_quizzes', progress: quizzes / 10, hint: '${10 - quizzes} more quizzes'));
    }
        ...
        child: Column(
          ...
          children: [
            const Row(children: [
              ...
              SizedBox(width: 6),
              Text('Next Up', style: TextStyle(color: Colors.amber, fontWeight: FontWeight.bold, fontSize: 13)),
            ]),
            const SizedBox(height: 10),
                ...
                child: Row(children: [
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      Text(hint.hint, style: const TextStyle(color: Colors.white30, fontSize: 9)),
                    ],
                  ),
                ]),
              );
...
class _MiniInfoChip extends StatelessWidget {
  ...
  const _MiniInfoChip({required this.icon, required this.text, required this.color});
...
class _MasterySummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Mastery', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.white)),
        const SizedBox(height: 10),
        ...levels.map((entry) {
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Text(level.label, style: TextStyle(color: level.color, fontSize: 11, fontWeight: FontWeight.w600)),
                          Text('$count', style: TextStyle(color: level.color, fontSize: 11, fontWeight: FontWeight.bold)),
                        ],
                      ),
...
class _UserGreeting extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final username = playerBox.get('cached_username', defaultValue: null) as String?;
    final displayName = (username != null && username.isNotEmpty) ? username : 'Doger';
    final playerState = ref.watch(playerProvider);
    final customPhotoPath = Hive.box('dogquest_player_stats').get('custom_avatar_path', defaultValue: '') as String;
    final isCustomPhoto = playerState.selectedAvatar == 'custom' && customPhotoPath.isNotEmpty && File(customPhotoPath).existsSync();
            ...
            children: [
              ...
              Positioned(
                ...
                child: Container(
                  ...
                  child: const Icon(Icons.edit, size: 10, color: Colors.white54),
                ),
              ),
            ],
          ),
  ...
  void _showAvatarPicker(BuildContext context, WidgetRef ref) {
    ...
    final customPath = Hive.box('dogquest_player_stats').get('custom_avatar_path', defaultValue: '') as String;
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (_, scrollController) => _AvatarPickerSheet(
          ...
          onPickPhoto: () async {
            ...
            Hive.box('dogquest_player_stats').put('custom_avatar_path', savedPath);
...
class _AvatarPickerSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasCustomPhoto = customPhotoPath.isNotEmpty && File(customPhotoPath).existsSync();
            ...
            itemBuilder: (_, i) {
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      color: hasCustomPhoto
                          ? Colors.teal.withValues(alpha: isSelected ? 0.3 : 0.1)
                    ...
                    child: Column(
                      ...
                      children: [
                        if (hasCustomPhoto)
                          ...
                          Icon(Icons.add_a_photo_rounded, color: Colors.white.withValues(alpha: 0.4), size: 28),
                        const SizedBox(height: 6),
                        Text(
                        ...
                        const SizedBox(height: 2),
                        Text(
                          hasCustomPhoto ? 'Tap to change' : 'Use your own photo',
              ...
              return GestureDetector(
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color: unlocked
                        ? avatar.bgColor.withValues(alpha: isSelected ? 0.3 : 0.1)
                  ...
                  child: Column(
                    ...
                    children: [
                      if (unlocked)
                        ...
                        Icon(Icons.lock_rounded, color: Colors.white.withValues(alpha: 0.15), size: 32),
                      const SizedBox(height: 6),
                      Text(
                        avatar.name,
                        style: TextStyle(
      ...
      return GestureDetector(
        ...
        child: Container(
          ...
          child: Row(children: [
            ...
            const Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Add Your Dog', style: TextStyle(color: Colors.amber, fontWeight: FontWeight.bold, fontSize: 16)),
                SizedBox(height: 2),
    ...
    return Column(
      children: [
        ...dogs.map((dog) => GestureDetector(
          onTap: () => context.push('/my-dog/profile/${Uri.encodeComponent(dog.name)}'),
          child: Container(
            width: double.infinity,
            margin: const EdgeInsets.only(top: 8, bottom: 4),
            ...
            child: Row(children: [
              ...
              ClipRRect(
                borderRadius: BorderRadius.circular(12),
                child: dog.photoPath != null && File(dog.photoPath!).existsSync()
                    ? Image.file(File(dog.photoPath!), width: 52, height: 52, fit: BoxFit.cover)
                    : Container(
                        width: 52, height: 52,
                        color: Colors.amber.withValues(alpha: 0.1),
              ...
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Row(children: [
                    if (dog.breed != null)
                      Text(dog.breed!, style: const TextStyle(color: Colors.amber, fontSize: 12)),
                    if (dog.breed != null && dog.ageYears != null)
                      const Text(' \u2022 ', style: TextStyle(color: Colors.white24, fontSize: 12)),
                    if (dog.ageYears != null)
                      Text('${dog.ageYears} yr${dog.ageYears == 1 ? '' : 's'}',
                        style: const TextStyle(color: Colors.white54, fontSize: 12)),
                  ]),
                  if (dog.daysUntilCelebration != null && dog.daysUntilCelebration! <= 30)
                    Padding(
                      ...
                      child: Container(
                        ...
                        child: Text(
                          ...
                          style: const TextStyle(color: Colors.pink, fontSize: 10, fontWeight: FontWeight.w600),
                        ),
                      ),
                    ),
                ]),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(Icons.add_circle_outline, color: Colors.white.withValues(alpha: 0.3), size: 16),
                const SizedBox(width: 6),
                Text('Add another dog', style: TextStyle(color: Colors.white.withValues(alpha: 0.3), fontSize: 12)),
              ]),
            ),
          ),
      ],
        ...
        child: Container(
          ...
          child: Row(children: [
            Container(
              ...
              child: const Icon(Icons.group_add, color: Color(0xFF7C4DFF), size: 22),
            ),
            const SizedBox(width: 14),
            const Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Start a Pack', style: TextStyle(color: Color(0xFF7C4DFF), fontWeight: FontWeight.bold, fontSize: 15)),
                SizedBox(height: 2),
                Text('Create a family group to share dogs & track stats together',
                    style: TextStyle(color: Colors.white54, fontSize: 12)),
              ]),
            ),
