...
class _FieldGuideScreenState extends ConsumerState<FieldGuideScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    var filtered = dogSvc.filter(rarity: _guideRarityFilter, search: _guideSearch);
              ...
              child: Row(children: [
                ...
                const Expanded(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('Dog Quiz', style: TextStyle(color: Colors.amber, fontWeight: FontWeight.bold)),
                    Text('Test your dog knowledge — earn bonus XP!',
            ...
            itemBuilder: (c, i) {
              ...
              return Card(
                ...
                shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(16),
                  side: BorderSide(color: dog.rarity.color.withValues(alpha: 0.4)),
                ),
                child: ListTile(
                    ...
                    Row(children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(10),
                          border: Border.all(color: dog.rarity.color.withValues(alpha: 0.5)),
                        ),
                        child: Text(dog.rarity.name,
                          style: TextStyle(color: dog.rarity.color, fontSize: 10)),
                      ),
                      const SizedBox(width: 8),
                      Text('+${dog.xp} XP', style: const TextStyle(color: Colors.amber, fontSize: 11)),
                    ]),
                  ]),
                  trailing: owned
                      ? const Icon(Icons.check_circle, color: Colors.green, size: 20)
                      : const Icon(Icons.chevron_right, color: Colors.white24),
                  onTap: () => DogDetailSheet.show(context, dog, _player, source: 'guide'),
                ),
              ).animate().fadeIn(delay: Duration(milliseconds: (i * 30).clamp(0, 300)));
            },
          ),
        ),
      ],
  ...
  Widget _collectedChip(bool collected, String label, Color color) {
    ...
    return Padding(
      ...
      child: GestureDetector(
        onTap: () => setState(() => _collectedFilter = selected ? null : collected),
