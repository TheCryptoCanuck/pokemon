...
class DogDetailSheet extends StatefulWidget {
  ...
  static void show(BuildContext context, Dog dog, AudioPlayer player, {String source = 'unknown'}) {
...
class _DogDetailSheetState extends State<DogDetailSheet> {
    ...
    return DraggableScrollableSheet(
          ...
          builder: (_, ctrl) => SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Center(child: Text(dog.scientificName,
                  ...
                  if (dog.audioUrl.isNotEmpty)
                    Expanded(
                      child: ElevatedButton.icon(
                        onPressed: () {
                          widget.player.setUrl(dog.audioUrl).then((_) => widget.player.play()).catchError((e) {
                          ...
                          showDialog(
                            ...
                            builder: (_) => AlertDialog(
                              ...
                              title: const Row(children: [
                                Text('\u{1F4A1}', style: TextStyle(fontSize: 24)),
                                SizedBox(width: 8),
                                Expanded(
  ...
  String _generateFunFact(Dog dog) {
    ...
    if (dog.conservationStatus == 'Critically Endangered') {
      facts.add('The ${dog.name} is critically endangered. This breed is extremely rare. Every sighting matters for breed preservation efforts.');
    } else if (dog.conservationStatus == 'Endangered') {
      facts.add('The ${dog.name} is listed as endangered. Breed preservation programs around the world are working to protect this breed.');
    } else if (dog.conservationStatus == 'Vulnerable') {
      facts.add('The ${dog.name} is considered vulnerable. Their numbers are declining, making each sighting increasingly valuable.');
    }
    if (dog.rarity == Rarity.legendary) {
      facts.add('The ${dog.name} is one of the rarest breeds you can find in DogQuest. Only 3% of encounters yield a legendary dog!');
    }
    if (dog.rarity == Rarity.rare) {
      facts.add('The ${dog.name} appears in only 12% of encounters. Finding one takes patience and a keen eye.');
    }
    facts.add('The scientific name "${dog.scientificName}" helps scientists worldwide identify this exact species regardless of language barriers.');
    facts.add('${dog.name} lives in ${dog.habitat.toLowerCase()}. Understanding habitats is key to successful dog spotting.');
