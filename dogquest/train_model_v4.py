...
v4 — FGVC (Fine-Grained Visual Categorization) enhancements:
  - Stanford Dogs BOUNDING BOX cropping (focuses model on dog, not background)
  - Mixed crop training (50% bbox crop, 50% full image) for inference robustness
  - Random Erasing augmentation (simulates occlusion — dogs behind furniture/fences)
  - CutMix batch augmentation (forces model to use body shape, not just face)
  - Bilinear pooling head (GAP * GMP captures texture/coat patterns)
  - EfficientNetB0 backbone with EfficientNet preprocessing
  - All v3 optimizations: CPU threading, .cache(), sample_from_datasets, label smoothing
...
FINE_TUNE_EPOCHS = 10       # fine-tuning (more epochs for FGVC)
FINE_TUNE_LAYERS = 40       # Unfreeze more layers for fine-grained features
OUTPUT_DIR = "assets"
SUPPLEMENTAL_DIR = "supplemental_dogs"
LABEL_SMOOTHING = 0.1

# FGVC-specific config
USE_BBOX_CROP = True         # Use Stanford Dogs bounding box annotations
BBOX_PADDING = 0.15          # 15% padding around bounding box
BBOX_MIXED_RATE = 0.5        # 50% cropped, 50% full image during training
USE_CUTMIX = True            # CutMix batch augmentation
...
RANDOM_ERASING_PROB = 0.3    # Per-image probability
USE_BILINEAR_HEAD = True     # Bilinear pooling (GAP * GMP) in model head
...
print(f"  CutMix: {USE_CUTMIX} (alpha={CUTMIX_ALPHA}, prob={CUTMIX_PROB})")
print(f"  Random erasing: {USE_RANDOM_ERASING} (prob={RANDOM_ERASING_PROB})")
print(f"  Bilinear head: {USE_BILINEAR_HEAD}")
...
(ds_train_stanford_raw, ds_test_stanford_raw), info = tfds.load(
    "stanford_dogs",
    split=["train", "test"],
    as_supervised=False,   # Full feature dict for bbox access
    with_info=True,
)
...
def crop_to_bbox_with_padding(image, bbox, padding=BBOX_PADDING):
    ...
    # Add padding (keeps ears, tails, body context)
    box_h = ymax - ymin
    box_w = xmax - xmin
    pad_h = box_h * padding
    pad_w = box_w * padding
...
def extract_with_mixed_crop(example):
    """50% bbox crop, 50% full image — teaches model to handle both."""
    image = example["image"]
    label = example["label"]
    bbox = example["objects"]["bbox"]

    # Check if bbox exists (should always for Stanford Dogs)
...
# Convert raw datasets to (image, label) with bbox handling
...
if os.path.isdir(SUPPLEMENTAL_DIR):
    for entry in sorted(os.listdir(SUPPLEMENTAL_DIR)):
        ...
        if not os.path.isdir(folder_path):
            continue
        # Collect images — use set to avoid double-counting .jpg/.JPG on Windows
        imgs_set = set()
...
def random_erasing(image, probability=RANDOM_ERASING_PROB):
    """Random Erasing — simulates occlusion (dogs behind furniture/fences).
    Fills erased region with random noise (better than zeros for FGVC)."""
    ...
    def do_erase():
        ...
        # Random erase area: 2-25% of image
        ...
        # Build mask: 1 everywhere except the erase region
        # Use padding approach for graph-mode compatibility
...
def augment(image, label):
    """FGVC-enhanced augmentation for training (standard breeds)."""
    image = tf.cast(image, tf.float32)
...
def augment_supplemental(image, label):
    """Stronger FGVC augmentation for supplemental images (fewer samples)."""
    image = tf.cast(image, tf.float32)
    ...
    image = tf.clip_by_value(image, 0.0, 255.0)
    # Random erasing with higher probability for supplemental (more regularization)
...
def cutmix_batch(images, labels):
    """CutMix: cut-and-paste regions between training images.
    Forces model to attend to body shape + coat, not just face."""
    ...
    # Build mask (1 = keep original, 0 = paste from shuffled)
    ...
    mask = tf.concat([top, middle_row, bottom], axis=0)  # [H, W, 3]
    ...
    # Mix labels (one-hot soft labels)
...
def maybe_cutmix(images, labels):
    """Apply CutMix with probability CUTMIX_PROB, otherwise convert to one-hot."""
    if USE_CUTMIX:
...
# Use sample_from_datasets for balanced interleaving
...
# --- Test/validation dataset (one-hot labels to match training) ---
def preprocess_to_onehot(image, label):
    """Preprocess and convert to one-hot for validation (matches CutMix output)."""
    image = tf.image.resize(image, [IMG_SIZE, IMG_SIZE])
    image = tf.cast(image, tf.float32)
    image = tf.keras.applications.efficientnet.preprocess_input(image)
...
def preprocess_supplemental_onehot(image, label):
    """Supplemental preprocess with one-hot labels."""
    image = tf.cast(image, tf.float32)
    image = tf.keras.applications.efficientnet.preprocess_input(image)
    label_oh = tf.one_hot(tf.cast(label, tf.int32), NUM_CLASSES)
    return image, label_oh
...
# ── Step 8: Build Model (Bilinear Pooling head for FGVC) ────────────────
print(f"\nBuilding EfficientNetB0 model ({NUM_CLASSES} classes, "
      f"{'bilinear' if USE_BILINEAR_HEAD else 'slim'} head)...")
...
if USE_BILINEAR_HEAD:
    # Bilinear pooling: GAP * GMP captures second-order texture/coat statistics
    # This is key for fine-grained tasks (distinguishing Norfolk vs Norwich Terrier)
    ...
    # Element-wise product (approximates bilinear interaction)
    bilinear = tf.keras.layers.Multiply(name="bilinear_mul")([gap, gmp])

    # Signed sqrt + L2 normalize (stabilizes bilinear features)
    bilinear = tf.keras.layers.Lambda(
        lambda z: tf.math.sign(z) * tf.math.sqrt(tf.math.abs(z) + 1e-12),
        name="signed_sqrt"
    )(bilinear)
    bilinear = tf.keras.layers.Lambda(
    ...
    # Concatenate first-order (GAP) + second-order (bilinear) features
...
# (works with both one-hot from CutMix and standard one-hot)
def categorical_crossentropy_with_smoothing(y_true, y_pred):
    """Categorical crossentropy with label smoothing for one-hot labels."""
...
early_stop_ft = tf.keras.callbacks.EarlyStopping(
    monitor="val_accuracy",
    patience=3,   # More patience for fine-tuning phase
    restore_best_weights=True,
    verbose=1,
)
...
for images, labels_oh in test_ds:
    ...
    # Convert one-hot back to sparse for confusion matrix
...
print(f"\n  FGVC features used:")
print(f"    Bbox crop: {USE_BBOX_CROP}")
print(f"    CutMix: {USE_CUTMIX}")
print(f"    Random erasing: {USE_RANDOM_ERASING}")
print(f"    Bilinear head: {USE_BILINEAR_HEAD}")
