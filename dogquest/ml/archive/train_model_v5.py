"""
Train a dog breed classifier — v5 targeting 90%+ accuracy.

v5 — Major accuracy push:
  - EfficientNetB2 backbone (better features, ~9 MB quantized vs B0's 5 MB)
  - RandAugment (proven +2-4% on fine-grained classification)
  - Rotation ±15°, hue/saturation jitter, random grayscale
  - Mixup + CutMix combined (both proven on FGVC)
  - Progressive resizing: 192 → 224 → 260 (curriculum learning)
  - Longer fine-tuning: 20 epochs with cosine warmup + restart
  - Unfreeze entire backbone in final phase (deeper fine-tuning)
  ...
  - Knowledge distillation: teacher model saves soft labels (optional phase)
...
FINE_TUNE_LAYERS_STAGE = [40, 80, -1]    # -1 = unfreeze all
...
RANDAUG_MAGNITUDE = 9       # magnitude (0-10 scale)
...
def build_train_pipeline(img_size):
    ...
    stanford_train = ds_train_stanford.cache().map(aug_fn, num_parallel_calls=tf.data.AUTOTUNE)
...
for stage_idx, (img_size, ft_epochs, ft_layers) in enumerate(
    ...
    base_lr = 5e-5 if stage_idx == 0 else (3e-5 if stage_idx == 1 else 1e-5)
...
report_path = "train_v5_report.json"
