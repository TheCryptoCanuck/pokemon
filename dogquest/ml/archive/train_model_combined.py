...
IMG_SIZE = 224
BATCH_SIZE = 16
EPOCHS = 10              # head training
FINE_TUNE_EPOCHS = 10    # fine-tuning top 50 layers (reduced from 15, early stopping handles the rest)
OUTPUT_DIR = "assets"
...
def augment_supplemental(image, label):
    """Augmentation for supplemental images (already float32, [0,255] range)."""
    image = tf.image.resize(image, [IMG_SIZE + 40, IMG_SIZE + 40])
    image = tf.image.random_crop(image, [IMG_SIZE, IMG_SIZE, 3])
    image = tf.image.random_flip_left_right(image)
    image = tf.image.random_brightness(image, 0.25)
